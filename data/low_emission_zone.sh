...
cat low_emission_zone_raw.geojson | jq -c '.features[] | select(.properties."access:note" != null) | . + {properties: (.properties + {low_emission_zone: "ture"})}' > low_emission_zone.geojson
